...
function insertGame(gameID, gameName, gameImageUrl) {
    const query = 'INSERT OR REPLACE INTO games (gameID, gameName, gameImageUrl) VALUES (?, ?, ?)';
...
function insertSale(gameID, discountAmount, oldPrice, salePrice, saleTimestamp) {
    const query = 'INSERT INTO sales (gameID, discountAmount, oldPrice, salePrice, saleTimestamp) VALUES (?, ?, ?, ?, ?)';
    db.run(query, [gameID, discountAmount, oldPrice, salePrice, saleTimestamp], (err) => {
        if (err) {
...
async function getExpiryDate(gameID) {
    try {
        ...
        let response = await axios.get(`https://store.steampowered.com/app/${gameID}`, {
            headers: {
        ...
        try {
            let promotionText = $('#game_area_purchase').find('.game_purchase_discount_countdown').first().text();
    ...
    try {
        ...
        $('.search_result_row').each(async (index, element) => {
            ...
            if (isApp) {
                ...
                game.discountAmount = $(element).find('.discount_block.search_discount_block').data('discount');
                game.oldPrice = $(element).find('.discount_original_price').text().trim();
                ...
                ifGameExists(game.gameID, (existingGame) => {
                    if (!existingGame || existingGame.gameImageUrl !== game.gameImageUrl) {
